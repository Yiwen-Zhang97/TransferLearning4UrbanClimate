...
file_to_store=f'_finetune_1_US_ERA5_{mode}_scratchAllFC_nofreeze_moreDrop{dropout_pct}_val{fold_val_num}_trainTot{fold_train_tot}_lessParam_skipcon_lst_2013-2024_new'
...
PATH_TO_MODEL=f"/glade/u/home/yiwenz/TransferLearning/New_Organized_Code/Train_test_results_model/best_resnet_more_linear_samesize_all_cities_LSTNorm_CL_6_L1_onehot_day_ERA5_US_monthly_0-1_all_cont_9.pt"
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Linear(128+256, out_features=64)
...
def load_pretrained_weights(model,path,freeze=True):
    ...
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if 'fc3' not in k and 'fc2' not in k and 'fc1' not in k} #if 'fc3' not in k and 'fc2' not in k and 'fc1' not in k
...
scheduler2 = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, "min", patience=patience,factor=factor)
